...
import { X, User, Lock, Eye, EyeOff, Shield, KeyRound, ArrowLeft, CheckCircle2, Loader2, AlertCircle } from 'lucide-react';
...
type Step = 'identify' | 'newpassword' | 'success';
...
export function RecoveryModal({ onClose, onBackToLogin }: RecoveryModalProps) {
  ...
  // Identify step — nickname + auto-lookup + verify in one screen
  ...
  // ─── Debounced auto-lookup of security question ───
  useEffect(() => {
    if (debounceRef.current) clearTimeout(debounceRef.current);

    const trimmed = nickname.trim();

    // Minimum length before attempting lookup
    if (trimmed.length < 3) {
    ...
    // Skip if we already fetched this exact nickname
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        ...
        if (!res.ok) {
          // For privacy on "user not found", we keep the existing API behavior (it already returns 404)
          setLookup({ status: 'error', message: data.error || (isEs ? 'No encontrado' : 'Not found') });
          return;
        }

        lastFetchedRef.current = trimmed;
        setLookup({ status: 'found', question: data.securityQuestion.label });
      } catch {
        setLookup({ status: 'error', message: isEs ? 'Error de conexión' : 'Connection error' });
      }
    }, 600);
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    if (!verifyInput.trim()) {
      toast.error(method === 'code' ? (isEs ? 'Ingresa el código' : 'Enter the code') : (isEs ? 'Ingresa tu respuesta' : 'Enter your answer'));
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(data.error || (isEs ? 'Verificación fallida' : 'Verification failed'));
        return;
      }
  ...
  const stepLabels: Record<Step, string> = {
    identify: isEs ? 'identificar cuenta' : 'identify account',
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
          ...
          <div className="flex justify-center gap-2 mb-2">
            {(['identify', 'newpassword', 'success'] as Step[]).map((s, i) => (
              <div key={s} className={`w-8 h-1 clip-btn transition-all duration-300 ${
                step === s
                  ? 'bg-[#FF2D78] shadow-[0_0_8px_rgba(255,45,120,0.4)]'
                  : ((['identify', 'newpassword', 'success'] as Step[]).indexOf(step) > i
                    ? 'bg-[#9d4edd]/40'
                    : 'bg-white/8'
                  )
              }`} />
          ...
          <p className="font-code text-[10px] text-center text-white/30 mb-6 italic">
            {step === 'identify' && (isEs ? 'Todo tiene un inicio...' : 'Everything has a beginning...')}
            {step === 'newpassword' && (isEs ? 'Un nuevo capítulo comienza...' : 'A new chapter begins...')}
            {step === 'success' && (isEs ? 'Renaciste como el fénix...' : 'You were reborn like the phoenix...')}
          </p>

          {/* ─── Step: Identify (nickname + auto-lookup + verify, all in one) ─── */}
          {step === 'identify' && (
            <form onSubmit={handleVerify} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text"
                    ...
                    value={nickname}
                    onChange={e => { setNickname(e.target.value); setVerifyInput(''); }}
                    required
                    minLength={3}
                    autoFocus
                    className="w-full pl-9 pr-10 py-2.5 bg-[#080812] border border-[#FF2D78]/15 text-white font-code text-sm placeholder:text-white/20 focus:outline-none focus:border-[#FF2D78]/40 focus:shadow-[0_0_10px_rgba(255,45,120,0.1)] transition-all"
                  />
                  {/* Status indicator inside input */}
                  {lookup.status === 'loading' && (
                    <Loader2 size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#FF2D78] animate-spin" />
                  )}
                  {lookup.status === 'found' && (
                    <CheckCircle2 size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-green-400" />
                  )}
                  {lookup.status === 'error' && (
                    <AlertCircle size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-red-400/70" />
                  )}
                </div>
              </div>

              {/* Auto-lookup result: error */}
              {lookup.status === 'error' && (
                <div className="p-2.5 clip-card bg-red-500/5 border border-red-500/15">
                  <div className="flex items-start gap-2">
                    <AlertCircle size={12} className="text-red-400/70 mt-0.5 shrink-0" />
                    <p className="font-code text-[10px] text-red-300/80 leading-relaxed">{lookup.message}</p>
                  </div>
                </div>
              )}

              {/* Auto-lookup result: question found → show inline verify panel */}
              {lookup.status === 'found' && (
                <motion.div
                ...
                >
                  {/* Show the security question automatically */}
                  <div className="p-3 clip-card bg-[#00F2FE]/5 border border-[#00F2FE]/15">
                    <p className="font-code text-[9px] text-white/40 mb-1 uppercase tracking-wider">
                      {isEs ? 'Tu pregunta de seguridad:' : 'Your security question:'}
                    </p>
                    <p className="font-code text-xs text-[#00F2FE] font-medium leading-relaxed">
                      {lookup.question}
                    </p>
                  </div>

                  {/* Method selector — user can switch to recovery code if they prefer */}
                  <div className="flex gap-2">
                    <button type="button"
                      onClick={() => { setMethod('question'); setVerifyInput(''); }}
                      className={`flex-1 clip-btn py-2 text-[11px] font-cyber font-bold flex items-center justify-center gap-1.5 transition-all uppercase tracking-wider ${
                        method === 'question'
                      ...
                      <Shield size={11} /> {isEs ? 'Pregunta' : 'Question'}
                    </button>
                    <button type="button"
                      ...
                      <KeyRound size={11} /> {isEs ? 'Código' : 'Code'}
                    </button>
                  </div>
                  ...
                  <div>
                    <label className="font-code text-[10px] font-bold text-white/35 uppercase tracking-widest block mb-1.5">
                      ...
                      {method === 'code' ? (isEs ? 'Código' : 'Code') : (isEs ? 'Respuesta' : 'Answer')}
                    </label>
                    <div className="relative">
                      {method === 'code' ? (
              ...
              {/* Hint when idle */}
              {lookup.status === 'idle' && (
                <p className="font-code text-[10px] text-white/25 text-center italic">
                  {isEs ? 'Escribe tu nickname para continuar' : 'Type your nickname to continue'}
                </p>
              )}
          ...
          {step === 'newpassword' && (
            <form onSubmit={handleResetPassword} className="space-y-4">
              <div>
                <label className="font-code text-[10px] font-bold text-white/35 uppercase tracking-widest block mb-1.5">
                  <span className="text-[#9d4edd]">{'>'}</span> {isEs ? 'Nueva contraseña' : 'New password'}
                </label>
                <div className="relative">
              ...
              <div>
                <label className="font-code text-[10px] font-bold text-white/35 uppercase tracking-widest block mb-1.5">
                  <span className="text-[#9d4edd]">{'>'}</span> {isEs ? 'Confirmar contraseña' : 'Confirm password'}
                </label>
                <div className="relative">
                  <Lock size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#9d4edd]/40" />
                  <input type="password"
              ...
              <button type="submit" disabled={loadingReset}
                className="w-full btn-primary text-sm py-3 uppercase tracking-wider disabled:opacity-50">
                {loadingReset ? <><Loader2 size={14} className="animate-spin" /> {t('common.loading')}</> : (isEs ? 'Cambiar Contraseña' : 'Change Password')}
              </button>

              <button type="button" onClick={() => { setStep('identify'); setNewPassword(''); setConfirmNewPassword(''); }}
                className="w-full py-2 font-code text-[11px] text-white/30 hover:text-white/50 flex items-center justify-center gap-2 transition-colors">
            ...
            <div className="space-y-4">
              <div className="p-4 clip-card bg-green-500/5 border border-green-500/15 text-center">
                <p className="font-code text-xs text-green-400 font-medium">
                  {isEs ? 'Tu contraseña ha sido actualizada exitosamente.' : 'Your password has been updated successfully.'}
                </p>
          ...
          <p className="text-center font-code text-[8px] text-white/15 mt-2 tracking-wider">
            THE ENCODERS CLUB &middot; {isEs ? 'RECUPERACIÓN SEGURA' : 'SECURE RECOVERY'}
          </p>
        </motion.div>
      </motion.div>
    </AnimatePresence>
