...
/* ─── Feature Carousel (MONIKA ONLY - DIAGONAL CARDS) ─── */
function FeatureCarousel() {
  const features = [
    { title: "Tu sala del club", image: "/features/room.jpg" },
    { title: "Menú de interacciones mejorado", image: "/features/menu.jpg" },
    { title: "Nuevos temas de conversación", image: "/features/topics.jpg" },
    { title: "Generador de diálogos", image: "/features/dialogue.jpg" },
  ];
  ...
  return (
    <motion.div
      ...
      className="relative"
    >
      <div ref={scrollRef} className="flex gap-4 overflow-x-auto scrollbar-hide snap-x scroll-smooth pb-4">
        {features.map((feature, idx) => (
          <div            key={idx}
            className="flex-none w-72 bg-white rounded-2xl border-2 border-[#FFB6C1] overflow-hidden shadow-sm hover:shadow-md transition-shadow snap-start"
            style={{ transform: `rotate(${(idx % 2 === 0 ? -2 : 2)}deg)` }}
          >
            <div className="aspect-video bg-gradient-to-br from-pink-100 to-pink-50 flex items-center justify-center">
              <ImageIcon className="w-12 h-12 text-[#FFB6C1]" />
            </div>
            <div className="p-4 text-center">
              <p className="text-[#d87093] font-bold text-[15px]" style={{ fontFamily: "'RifficFree', monospace" }}>
                {feature.title}
              </p>
            </div>
          </div>
      ...
      <button onClick={() => scroll('left')} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 w-8 h-8 rounded-full bg-white border-2 border-[#FFB6C1] text-[#d87093] flex items-center justify-center hover:bg-pink-50 z-10 shadow-sm">
        <ChevronLeft size={16} />
      </button>
      <button onClick={() => scroll('right')} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 w-8 h-8 rounded-full bg-white border-2 border-[#FFB6C1] text-[#d87093] flex items-center justify-center hover:bg-pink-50 z-10 shadow-sm">
...
/* ─── Genre Badges (MONIKA ONLY - COLORED TAGS) ─── */
function GenreBadges() {
  const genres = [
    { name: "Fan-Made", color: "bg-[#FFB6C1] text-[#d6336c]" },
    { name: "Drama", color: "bg-[#7FFFD4] text-[#008B8B]" },
    { name: "Romance", color: "bg-[#DDA0DD] text-[#8B008B]" },
    { name: "Simulación", color: "bg-[#ADD8E6] text-[#4682B4]" },
  ];
  ...
  return (
    ...
    >
      {genres.map((genre, idx) => (
        <div
          key={idx}
          className={`px-6 py-2.5 rounded-full border-2 border-white shadow-sm ${genre.color}`}
          style={{
            fontFamily: "'RifficFree', monospace",
            fontSize: '14px',
            fontWeight: 'bold',            transform: `rotate(${(idx % 2 === 0 ? -1 : 1)}deg)`,
          }}
        >
          {genre.name}
        </div>
      ...
      <AnimatePresence>
        {lightboxIdx !== null && (
          <motion.div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={closeLightbox}>
            ...
            {lightboxIdx > 0 && <button onClick={e => { e.stopPropagation(); prevImage(); }} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/10 border border-white/20 text-white flex items-center justify-center hover:bg-white/20 transition-all z-10"><ChevronLeft size={22} /></button>}            {lightboxIdx < images.length - 1 && <button onClick={e => { e.stopPropagation(); nextImage(); }} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/10 border border-white/20 text-white flex items-center justify-center hover:bg-white/20 transition-all z-10"><ChevronRight size={22} /></button>}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="rounded-2xl overflow-hidden border aspect-video relative group" style={{ borderColor: `${project.themeColor}80` }}>
              <img src={project.image} alt={project.name} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />              <div className="absolute inset-0 bg-gradient-to-t from-[#0a0a1a]/30 to-transparent" />
          ...
          <div className="space-y-8">
            <div className="p-8 rounded-3xl bg-gradient-to-b from-white/10 to-transparent border border-white/10 backdrop-blur-xl sticky top-32 space-y-6">
              ...
              </ul>              <div className="border-t border-white/10 pt-6 space-y-3">
...
/* ─── Light/pink theme detail view — MONIKA (WITH NEW SECTIONS) ─── */
  ...
  return (
    <>
      <style>{`
        ...
        .pink-stroke-lg { font-family: 'RifficFree', 'm1_fixed', monospace; color: #fefefe; -webkit-text-stroke: 6px #ba609e; paint-order: stroke fill; }        .pink-stroke-sm { font-family: 'RifficFree', 'm1_fixed', monospace; color: #fefefe; -webkit-text-stroke: 5px #ba609e; paint-order: stroke fill; }
      ...
      <div className="relative z-10 min-h-screen w-full overflow-hidden" style={{ fontFamily: "'m1_fixed', monospace", backgroundColor: '#ffffff' }}>
        ...
        <main className="relative z-10 w-full max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8">
          ...
          {/* ✅ NUEVA SECCIÓN: CARACTERÍSTICAS DIAGONALES */}
          <FeatureCarousel />
          
          {/* ✅ NUEVA SECCIÓN: GÉNEROS CON COLORES */}
          <GenreBadges />
...
/* ── Light/pink theme detail view — NATSUKI (STRONGER PINK) ── */function NatsukiDetail({ project }: { project: typeof projects[number] }) {
  const { t, locale } = useI18n();
  const musicRef = useRef<HTMLIFrameElement>(null);
  const [muted, setMuted] = useState(false);
  ...
  return (
    <>
      ...
      <div className="relative z-10 min-h-screen w-full overflow-hidden" style={{ fontFamily: bodyFontFamily, backgroundColor: '#ffffff' }}>
        ...
        <main className="relative z-10 w-full max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8">
          <div className="space-y-4">
            <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>              <h1 className="natsuki-title text-4xl sm:text-5xl lg:text-6xl font-black leading-tight">{project.name}</h1>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="bg-white/85 rounded-2xl border-2 border-[#FF7EB3] p-5 shadow-sm space-y-5">
            ...
            <ul className="space-y-2.5">
              {[
                ...
                const ItemIcon = item.icon;
                return (
                  <li key={item.label} className="flex items-center gap-2 text-[14px]">                    <ItemIcon className="w-4 h-4 text-[#D63384] flex-shrink-0" />
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="bg-white/85 rounded-2xl border-2 border-[#9B59B6] p-5 shadow-sm space-y-5">
            ...
            <div className="border-t border-[#9B59B6]/50 pt-4 space-y-2">
              ...
              {project.downloads.map((dl, i) => {
                ...
                const stroke = strokeColors[i] || '#333';
                return (
                  <a key={i} href={dl.url} target="_blank" rel="noopener noreferrer" className="w-full py-3 rounded-2xl flex items-center justify-center gap-2.5 transition-all hover:brightness-110 hover:scale-[1.02] active:scale-[0.98] group shadow-md" style={{ background: dl.color, border: `3px solid ${stroke}` }}>                    <Icon className="w-4 h-4 group-hover:scale-110 transition-transform flex-shrink-0" style={{ color: dl.textColor || '#fff', filter: `drop-shadow(0 0 1px ${stroke})` }} />
                    <span className="font-black uppercase tracking-wide text-[19px]" style={{ color: dl.textColor || '#ffffff', WebkitTextStroke: `1.5px ${stroke}`, paintOrder: 'stroke fill' }}>{isEs ? dl.label : (dl.labelEn || dl.label)}</span>
                  </a>
                );
              })}
