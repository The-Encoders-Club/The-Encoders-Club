...
const NatsukiProjectView: React.FC<ProjectViewProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div
      className={`fixed inset-0 z-[100] bg-[#1a0a1a] text-white overflow-y-auto natsuki-theme transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}
    >
      {/* FONDO TEMÁTICO */}
      <div 
        className="fixed inset-0 z-0 pointer-events-none"
        style={{ background: 'linear-gradient(135deg, rgba(26, 10, 26, 0.95) 0%, rgba(255, 45, 120, 0.12) 50%, rgba(26, 10, 26, 0.95) 100%)' }}
      />
      ...
      <div className="relative z-10 min-h-screen w-full overflow-x-hidden">
        <nav className="sticky top-0 z-50 bg-[#1a0a1a]/95 border-b border-[#FF2D78]/30 px-4 sm:px-6 py-4 flex justify-between items-center w-full">
          <button 
            onClick={onClose}
        ...
        <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-12">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12 w-full">
            <div className="lg:col-span-2 space-y-8">
              <header>
                <h1 className="text-4xl sm:text-6xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-[#FF2D78] to-[#FF9EBC] mb-4">
                  Just Natsuki
                </h1>
            ...
            <div className="space-y-8">
              <div className="p-8 rounded-3xl bg-gradient-to-b from-white/10 to-transparent border border-white/10 sticky top-32">
                ...
                <a 
                  ...
                  className="w-full py-4 rounded-2xl bg-gradient-to-r from-[#FF2D78] to-[#FF9EBC] text-white font-black uppercase tracking-tighter flex items-center justify-center gap-3 hover:shadow-[0_0_30px_rgba(255,45,120,0.4)] transition-shadow"
                >
