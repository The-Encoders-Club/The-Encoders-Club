...
const MonikaProjectView: React.FC<ProjectViewProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div
      className={`fixed inset-0 z-[100] bg-[#0a0a1a] text-white overflow-y-auto monika-theme transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}
    >
      {/* FONDO TEMÁTICO */}
      <div 
        className="fixed inset-0 z-0 pointer-events-none"
        style={{ background: 'linear-gradient(135deg, rgba(10, 10, 26, 0.95) 0%, rgba(255, 45, 120, 0.15) 50%, rgba(10, 10, 26, 0.95) 100%)' }}
      />
      ...
      <div className="relative z-10 min-h-screen w-full overflow-x-hidden">
        <nav className="sticky top-0 z-50 bg-[#0a0a1a]/95 border-b border-[#FF2D78]/30 px-4 sm:px-6 py-4 flex justify-between items-center w-full">
          <button 
            onClick={onClose}
          ...
          <button className="p-2 rounded-full bg-[#FF2D78]/20 border border-[#FF2D78]/30 text-[#FF2D78] hover:bg-[#FF2D78] hover:text-white transition-colors">
        ...
        <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-12">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12 w-full">
            <div className="lg:col-span-2 space-y-8">
              <header>
                <h1 className="text-4xl sm:text-6xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-[#FF2D78] to-[#00F3FF] mb-4">
                ...
                <div className="flex gap-4 border-b border-white/10 pb-4">
                  {['info', 'kiwi'].map((tab) => (
                    <button
                      ...
                      className={`px-6 py-2 rounded-lg font-bold uppercase tracking-widest transition-all duration-200 text-sm ${
                        activeTab === tab 
                        ...
                        <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide snap-x">
                          {previewImages.map((src, idx) => (
                            <div key={idx} className="flex-none w-64 rounded-xl overflow-hidden border border-white/10 aspect-video group relative snap-start">
                              <img src={src} alt={`Vista Previa ${idx + 1}`} loading="lazy" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" />
              ...
              <div className="p-8 rounded-3xl bg-gradient-to-b from-white/10 to-transparent border border-white/10 sticky top-32 space-y-6">
                ...
                <div className="border-t border-white/10 pt-6 space-y-3">
                  ...
                  <a 
                    ...
                    className="w-full py-3 rounded-lg bg-gradient-to-r from-[#FF2D78] to-[#FF6B9D] text-white font-bold uppercase tracking-tighter flex items-center justify-center gap-2 hover:shadow-[0_0_20px_rgba(255,45,120,0.5)] transition-shadow text-sm"
                  >
                  ...
                  <a 
                    ...
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="w-full py-3 rounded-lg bg-gradient-to-r from-[#00F3FF] to-[#00D9FF] text-[#0a0a1a] font-bold uppercase tracking-tighter flex items-center justify-center gap-2 hover:shadow-[0_0_20px_rgba(0,243,255,0.5)] transition-shadow text-sm"
                  >
                  ...
                  <a 
                    ...
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="w-full py-3 rounded-lg bg-gradient-to-r from-[#a855f7] to-[#d946ef] text-white font-bold uppercase tracking-tighter flex items-center justify-center gap-2 hover:shadow-[0_0_20px_rgba(168,85,247,0.5)] transition-shadow text-sm"
                  >
